        ...
        return (
            <div className="p-4 md:p-8 lg:p-12 flex items-center justify-center min-h-[400px]">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-slate-500 font-bold uppercase tracking-widest">Loading Categories...</p>
                </div>
            </div>
        );
    }
        ...
        <div className="p-4 md:p-8 lg:p-12 space-y-10 relative">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="space-y-2">
                    ...
                    <h2 className="font-bold text-slate-800 tracking-tight">Product Categories</h2>
                    <p className="text-slate-500 font-medium max-w-2xl leading-relaxed">Organize your products into meaningful groups.</p>
                </div>

                <div className="flex items-center gap-3">
                    <button
                        onClick={() => navigate("/products")}
                        className="flex items-center justify-center gap-2 px-6 py-4 bg-white border border-slate-200 text-slate-700 font-bold rounded-2xl shadow-sm hover:bg-slate-50 transition-all active:scale-95 group leading-none"
                    >
            ...
            <div id="category-form" className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden p-8 scroll-mt-10">
                <div className="mb-6 flex items-center justify-between">
                    <div>
                        <h3 className="font-bold text-[#172b4d]">{formData.id ? "Edit Category" : "Add New Category"}</h3>
                        <p className="text-slate-400 font-bold uppercase tracking-widest leading-none mt-1">
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-start">
                    ...
                    <div className="lg:row-span-2">
                        <label className="font-bold text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Category Image</label>
                            ...
                            >
                                {imagePreview ? (
                                    ...
                                    <div className="flex flex-col items-center p-4">
                                        ...
                                        <p className="font-bold text-slate-400 mt-2 uppercase tracking-tighter">Upload</p>
                                    </div>
                                )}
                            </label>
                        </div>
                    ...
                    <div className="lg:col-span-3 grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                            <label className="font-bold text-slate-400 uppercase tracking-widest ml-1">Category Name <span className="text-red-500">*</span></label>
                            <input
                                type="text"
                                value={formData.name}
                                ...
                                required
                                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 font-bold text-[#172b4d] focus:outline-none focus:ring-4 focus:ring-primary/5 focus:border-primary placeholder:text-slate-300 transition-all shadow-sm"
                            />
                        </div>

                        <div className="space-y-2">
                            <label className="font-bold text-slate-400 uppercase tracking-widest ml-1">Parent Category</label>
                            <div className="relative group">
                                <select
                                    value={formData.parent_id}
                                    onChange={(e) => setFormData({ ...formData, parent_id: e.target.value })}
                                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 font-bold text-[#172b4d] focus:outline-none focus:ring-4 focus:ring-primary/5 focus:border-primary transition-all appearance-none cursor-pointer shadow-sm"
                                >
                    ...
                    <div className="lg:col-span-3 flex items-center justify-end gap-3 mt-4">
                        <button
                            type="button"
                            onClick={handleReset}
                            className="px-6 py-3 text-slate-500 font-bold hover:bg-slate-50 rounded-xl transition-all uppercase tracking-[0.2em]"
                        >
                            Discard
                        </button>
                        <button
                            type="submit"
                            disabled={saving}
                            className={`px-8 py-3.5 bg-[#172b4d] text-white font-bold rounded-xl shadow-xl shadow-[#172b4d]/20 hover:bg-[#1a335a] transition-all active:scale-95 flex items-center gap-3 uppercase tracking-[0.2em] ${saving ? "opacity-50 grayscale" : ""}`}
                        >
                            {saving ? (
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden min-h-[400px]">
                <div className="p-6 md:p-8 border-b border-slate-50 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    ...
                    <p className="font-bold text-slate-400 uppercase tracking-widest">Showing {filteredCategories.length} categories</p>
                </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left min-w-[800px]">
                        <thead className="bg-slate-50/70 border-b border-slate-50">
                            <tr>
                                <th className="px-8 py-5 font-bold text-slate-400 uppercase tracking-widest">ID</th>
                                <th className="px-8 py-5 font-bold text-slate-400 uppercase tracking-widest">IMAGE</th>
                                <th className="px-8 py-5 font-bold text-slate-400 uppercase tracking-widest">NAME</th>
                                <th className="px-8 py-5 font-bold text-slate-400 uppercase tracking-widest">PARENT</th>
                        ...
                        <tbody className="divide-y divide-slate-50">
                            {filteredCategories.map((cat) => (
                                <tr key={cat.id} className={`hover:bg-slate-50/50 transition-colors ${formData.id === cat.id ? "bg-primary/5" : ""}`}>
                                    <td className="px-8 py-6 font-bold text-slate-400">#{cat.id}</td>
                                    ...
                                    <td className="px-8 py-6">
                                        <h4 className="font-bold text-[#172b4d]">{cat.name}</h4>
                                    </td>
                                    <td className="px-8 py-6">
                                        {cat.parent_id ? (
                                            <div className="flex items-center gap-2">
                                                <span className="w-1 h-1 rounded-full bg-primary/40"></span>
                                                <span className="font-bold text-[#172b4d] uppercase tracking-wider">
                                                    {categories.find(c => c.id === cat.parent_id)?.name || `ID: ${cat.parent_id}`}
                                                </span>
                                            </div>
                                        ) : (
                                            <span className="text-slate-300 font-bold uppercase tracking-[0.2em]">Top Level</span>
                                        )}
                                    </td>
                                    <td className="px-8 py-6">
                                        <p className="font-bold text-slate-400 uppercase tracking-widest">{new Date(cat.created_at).toLocaleDateString(undefined, { day: '2-digit', month: 'short', year: 'numeric' })}</p>
                                    </td>
