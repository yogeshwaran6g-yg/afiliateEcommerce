...
export default function OrderTable({ orders, expandedOrder, setExpandedOrder, orderDetails, pagination }) {
  ...
  const renderPageNumbers = () => {
    const pages = [];
    const maxVisible = 3; // On mobile, show fewer

    if (totalPages <= 5) {
      for (let i = 1; i <= totalPages; i++) pages.push(i);
    } else {
      // Logic for Ellipsis (simplified)
      if (currentPage <= 3) {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
      ...
      {/* Pagination Controls */}
      {totalPages > 1 && (
        <div className="px-4 md:px-6 py-4 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between bg-slate-50/50 gap-4">
          <div className="text-[10px] md:text-sm text-slate-500 font-medium order-2 sm:order-1">
