...
const Signup = () => {
  ...
  const [referralForcedEditable, setReferralForcedEditable] = useState(false);

  const referralParam =
    searchParams.get("ref") || searchParams.get("referralId");

  useEffect(() => {
    if (referralParam && !referralForcedEditable) {
      setFormData((prev) => ({ ...prev, referralId: referralParam }));
    }
  }, [referralParam, referralForcedEditable]);

  const isReferralReadOnly = !!referralParam && !referralForcedEditable;
              ...
              <div className="mt-1 relative">
                ...
                {isReferralReadOnly && (
                  <button
                    type="button"
                    onClick={() => {
                      setReferralForcedEditable(true);
                      setFormData((prev) => ({ ...prev, referralId: "" }));
                    }}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-primary hover:text-primary/70 text-xs font-bold"
                    title="Clear referral to edit"
                  >
                    Edit
                  </button>
                )}
