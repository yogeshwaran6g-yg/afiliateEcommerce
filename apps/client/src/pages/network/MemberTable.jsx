...
const MemberTable = ({ members, isLoading, error, pagination, onPageChange, onLimitChange }) => {
    ...
    if (isLoading && (!members || members.length === 0)) {
        return (
            <div className="p-12 text-center">
                <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-slate-100 text-slate-400 mb-4 animate-pulse">
                    <span className="material-symbols-outlined text-2xl">sync</span>
                </div>
                <h3 className="text-lg font-bold text-slate-900 mb-1">Loading members...</h3>
                <p className="text-sm text-slate-500">Fetching your team details from the server.</p>
            </div>
        );
    }
        ...
        <>
            <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody className={`divide-y divide-slate-100 ${isLoading ? 'opacity-50 pointer-events-none' : ''}`}>
                        {members.map((member) => (
                            <tr
                                key={member.id}
