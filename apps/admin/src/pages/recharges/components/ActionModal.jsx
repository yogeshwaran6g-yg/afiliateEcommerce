...
const ActionModal = ({ 
    ...
    if (!isOpen || !selectedRecharge) return null;
            ...
            <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 border border-slate-100">
                <div className="p-8 md:p-12 space-y-8">
                    <div className="flex items-center justify-between">
                        <div>
                            <h3 className={`text-3xl font-black tracking-tight ${isApprove ? 'text-emerald-600' : 'text-rose-600'}`}>
                                {isApprove ? 'Approve Recharge' : 'Reject Recharge'}
                            </h3>
                            <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.2em] mt-2">
                                Request ID: RECH-{selectedRecharge.id}
                            </p>
                        </div>
                        <button 
                            onClick={onClose} 
                    ...
                    <div className="bg-slate-50 rounded-[2rem] p-8 space-y-6 border border-slate-100 shadow-inner">
                        <div className="flex flex-col gap-1.5">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Recharge Amount</label>
                            <p className="text-3xl font-black text-slate-900 tracking-tight">
                                ₹{Number(selectedRecharge.amount).toLocaleString('en-IN')}
                            </p>
                        </div>
                        <div className="h-px bg-slate-200/50" />
                        <div className="flex flex-col gap-1.5">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">User / Distributor</label>
                            <div className="flex items-center gap-3 mt-1">
                                <div className="w-8 h-8 rounded-lg bg-white shadow-sm flex items-center justify-center text-primary font-black text-[10px] border border-slate-100">
                                    {selectedRecharge.user_name?.substring(0, 1).toUpperCase()}
                                </div>
                                <p className="text-sm font-black text-slate-700">{selectedRecharge.user_name}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center justify-between px-1">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Admin Notes</label>
                            <span className="text-[9px] text-slate-300 font-bold uppercase">Required for rejection</span>
                        </div>
                        <textarea
                            ...
                            rows={4}
                            placeholder={isApprove ? "Notes for approval (optional)..." : "Reason for rejection (required)..."}
                            value={adminComment}
