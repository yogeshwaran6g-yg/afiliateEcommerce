            ...
            <form onSubmit={handleSubmit} className="relative w-full max-w-2xl bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
                            ...
                            {imagePreview ? (
                                <>
                                    ...
                                    <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                        <button type="button" onClick={() => document.getElementById('image-upload').click()} className="px-6 py-3 bg-white text-slate-900 text-sm font-black rounded-xl shadow-xl hover:scale-105 transition-transform">
                                            Replace Image
                                        </button>
                                    </div>
                                </>
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="space-y-6">
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-[#172b4d] ml-1">Heading <span className="text-red-500">*</span></label>
                                <input
                                    type="text"
                                    name="heading"
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-[#172b4d] ml-1">Full Content <span className="text-red-500">*</span></label>
                                <textarea
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-[#172b4d] ml-1">Expiration Date & Time <span className="text-red-500">*</span></label>
                                <input
                                    type="datetime-local"
                                ...
                                />
                                <p className="text-[10px] text-slate-400 font-medium ml-1">When should this announcement stop showing?</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-8 bg-white/80 backdrop-blur-md border-t border-slate-50 flex items-center justify-end gap-4 z-20">
                    ...
                    <button type="submit" disabled={loading} className="px-8 py-3.5 bg-primary text-white text-sm font-black rounded-2xl shadow-xl shadow-primary/20 hover:bg-primary/90 transition-all hover:scale-105 active:scale-95 leading-none disabled:opacity-50">
                        {loading ? "Processing..." : (announcement ? "Update Announcement" : "Final Submit")}
                    </button>
                </div>
            </form>
        </div>
        ...
        return (
            <div className="p-4 md:p-8 lg:p-12 flex items-center justify-center min-h-[400px]">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-slate-500 font-bold uppercase tracking-widest text-[10px]">Retrieving Announcements...</p>
                </div>
            </div>
        );
    }
            ...
            <div className="flex flex-col xl:flex-row xl:items-start justify-between gap-8">
                <div className="space-y-4">
                    <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">
                        ...
                        <span className="text-primary font-black">Announcements</span>
                    </div>
                    <h2 className="text-3xl md:text-5xl font-black text-[#172b4d] tracking-tighter">Company Bulletin</h2>
                    <p className="text-sm md:text-xl text-slate-500 font-medium max-w-2xl">Manage system-wide announcements and broadcasts for all distributors.</p>
                </div>
                                ...
                                <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                                    <td className="px-10 py-6">
                                        <div className="flex items-center gap-6">
                                            <div className="w-20 h-14 rounded-xl bg-slate-100 border border-slate-200 overflow-hidden shrink-0 shadow-inner">
                                                {item.image_url ? (
                                            ...
                                            <div className="min-w-0">
                                                <h4 className="text-sm font-black text-[#172b4d] tracking-tight">{item.heading}</h4>
                                                <p className="text-[11px] text-slate-400 font-medium truncate mt-1">{item.short_description}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-10 py-6">
                                        <div className="space-y-1">
                                            ...
                                            <p className="text-sm font-bold text-[#172b4d]">{new Date(item.advertisement_end_time).toLocaleDateString()} {new Date(item.advertisement_end_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                                        </div>
                                    </td>
                                    <td className="px-10 py-6">
                                        {new Date(item.advertisement_end_time) > new Date() ? (
                                            <span className="px-3 py-1.5 rounded-xl bg-green-100 text-green-700 text-[10px] font-black uppercase tracking-widest">Active</span>
                                        ) : (
                                            <span className="px-3 py-1.5 rounded-xl bg-slate-100 text-slate-500 text-[10px] font-black uppercase tracking-widest">Expired</span>
                                        )}
                                    </td>
                                    <td className="px-10 py-6">
