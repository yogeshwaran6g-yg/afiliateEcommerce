...
export default function WithdrawFundsForm({ onClose, wallet }) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (numericAmount > maxWithdrawable) {
            setError(`You cannot withdraw more than your available balance (₹${maxWithdrawable.toFixed(2)}).`);
            return;
        }
        ...
        <div className="bg-white rounded-2xl p-6 md:p-8 shadow-xl border border-slate-200 relative overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-500">Transfer earnings to your bank account</p>
                    </div>
                </div>
                <button 
                    onClick={onClose}
            ...
            <div className="bg-slate-50 rounded-xl p-4 mb-6 border border-slate-100">
                <div className="flex justify-between items-center">
                    ...
                    <span className="text-xl font-bold text-primary">₹{maxWithdrawable.toFixed(2)}</span>
                </div>
            </div>

            <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-1.5">
                    <label className="text-sm font-bold text-slate-700">Withdrawal Amount (₹)</label>
