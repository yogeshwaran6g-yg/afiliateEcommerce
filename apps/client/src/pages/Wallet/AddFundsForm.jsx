...
export default function AddFundsForm() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            alert('Funds request submitted successfully! An admin will review your payment shortly.');
            navigate('/wallet');
        } catch (error) {
            alert(error.response?.data?.message || 'Failed to submit payment request. Please try again.');
        }
    };
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {formData.paymentMethod === 'gpay' && (
                    <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl flex flex-col items-center animate-in slide-in-from-top-2 duration-300">
                        <p className="text-sm font-bold text-slate-700 mb-3 text-center">Scan QR Code to pay via GPay</p>
                        <div className="w-48 h-48 bg-white border border-slate-200 rounded-2xl flex items-center justify-center mb-2 shadow-inner">
                            <div className="text-slate-400 flex flex-col items-center">
                                <span className="material-symbols-outlined text-5xl mb-1 text-primary">qr_code_2</span>
                                <span className="text-xs font-black tracking-widest text-slate-400">GPAY QR CODE</span>
                            </div>
                        </div>
                        <p className="text-xs text-slate-500 font-medium">Merchant: AI Affiliate Ecom</p>
                    </div>
                )}

                {formData.paymentMethod === 'phonepe' && (
                    <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl flex flex-col items-center animate-in slide-in-from-top-2 duration-300">
                        <p className="text-sm font-bold text-slate-700 mb-3 text-center">Scan QR Code to pay via PhonePe</p>
                        <div className="w-48 h-48 bg-white border border-slate-200 rounded-2xl flex items-center justify-center mb-2 shadow-inner">
                            <div className="text-slate-400 flex flex-col items-center">
                                <span className="material-symbols-outlined text-5xl mb-1 text-indigo-600">qr_code_2</span>
                                <span className="text-xs font-black tracking-widest text-indigo-600/50">PHONEPE QR CODE</span>
                            </div>
                        </div>
                        <p className="text-xs text-slate-500 font-medium">Merchant: AI Affiliate Ecom</p>
                    </div>
                ...
                {formData.paymentMethod === 'bank_transfer' && (
                    <div className="p-5 bg-slate-50 border border-slate-200 rounded-xl space-y-4 animate-in slide-in-from-top-2 duration-300 shadow-inner">
                        <p className="text-sm font-bold text-slate-700 text-center">Bank Transfer Details</p>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-3 gap-x-6 text-sm">
                            <div className="flex flex-col">
                                <span className="text-slate-400 font-bold text-[10px] uppercase tracking-wider">Bank Name</span>
                                <span className="font-bold text-slate-800">State Bank of India</span>
                            </div>
                            <div className="flex flex-col">
                                <span className="text-slate-400 font-bold text-[10px] uppercase tracking-wider">IFSC Code</span>
                                <span className="font-bold text-slate-800">SBIN0001234</span>
                            </div>
                            <div className="flex flex-col">
                                <span className="text-slate-400 font-bold text-[10px] uppercase tracking-wider">Account Number</span>
                                <span className="font-bold text-slate-800 tracking-wider">30012345678</span>
                            </div>
                            <div className="flex flex-col">
                                <span className="text-slate-400 font-bold text-[10px] uppercase tracking-wider">Account Type</span>
                ...
                {(formData.paymentMethod !== '') && (
                    <div className="space-y-1.5 pt-2">
                        ...
                        <div className={`mt-1 flex justify-center px-6 pt-5 pb-6 border-2 ${errors.payslip ? 'border-red-300 bg-red-50/30' : 'border-slate-300 bg-slate-50'} border-dashed rounded-2xl hover:border-primary transition-all cursor-pointer relative group`}>
                            <div className="space-y-1 text-center">
                                {payslipPreview ? (
                                    <div className="relative inline-block">
                                        <img src={payslipPreview} alt="Preview" className="mx-auto h-40 w-auto rounded-xl shadow-lg border-2 border-white" />
                                        <button
                                            type="button"
                                            onClick={() => { setPayslipPreview(null); setFormData(p => ({ ...p, payslip: null })); }}
                        ...
                        {errors.payslip && <p className="text-xs text-red-500 font-medium mt-1">{errors.payslip}</p>}
                    </div>
                )}
