...
export default function WalletStats({ wallet, transactions = [] }) {
    ...
    const growth = useMemo(() => {
        if (!transactions || transactions.length === 0) return { percent: 0, trend: 'up' };
        ...
        const referralCommissions = transactions.filter(t => t.transaction_type === 'REFERRAL_COMMISSION');
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
            ...
            <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm relative overflow-hidden">
                ...
                <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-sm font-semibold text-slate-500 uppercase tracking-wider">Commission</span>
                </div>
                <div className="flex items-baseline gap-2">
                    <span className="text-3xl font-black text-slate-900 dark:text-white">
                        {
                            (formatCurrency(wallet?.commissions)) ||
                        <span className='text-red-600 bg-red-600/10 p-1  rounded-lg'>!fetch error</span>
                        }
                    </span>
                    <span className={`text-xs font-bold ${growth.trend === 'up' ? 'text-green-500 bg-green-500/10' : 'text-red-500 bg-red-500/10'} px-1.5 py-0.5 rounded`}>
                        {growth.trend === 'up' ? '+' : '-'}{growth.percent}%
                    </span>
                </div>
                <p className="text-xs text-slate-400 mt-2">Earned in the last 30 days</p>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm relative overflow-hidden">
                ...
                <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-sm font-semibold text-slate-500 uppercase tracking-wider">On Hold</span>
                </div>
                <div className="flex items-baseline gap-2">
                    <span className="text-3xl font-black text-slate-900 dark:text-white">
                        {
                ...
                <p className="text-xs text-slate-400 mt-2">Pending verification for payout <br/> or anyother else</p>
            </div>
            ...
            <div className="bg-[#1754cf] p-6 rounded-xl shadow-xl shadow-blue-500/20 relative overflow-hidden group">
                ...
                <div className="flex items-center gap-2 mb-4 relative z-10">
                    <div className="w-8 h-8 rounded-lg bg-white/20 flex items-center justify-center">
                        <span className="material-symbols-outlined text-white text-xl">wallet</span>
                    </div>
                    <span className="text-sm font-semibold text-white/80 uppercase tracking-wider">Withdrawable</span>
                </div>
                ...
                <p className="text-xs text-white/60 mt-2 relative z-10">Instant payout available</p>
            </div>
        </div>
    );
}
