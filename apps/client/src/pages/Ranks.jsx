...
export default function Ranks() {
    ...
    const careerRoadmap = [
        ...
        { rank: "DIAMOND", status: "CURRENT", color: "bg-primary", textColor: "text-primary", current: true },
    ...
    return (
        <div className="p-4 md:p-8 space-y-8">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="relative flex-shrink-0">
                            <div className="w-24 h-24 md:w-32 md:h-32 rounded-full border-4 md:border-8 border-primary/20 flex items-center justify-center">
                                <div className="w-16 h-16 md:w-24 md:h-24 rounded-full border-2 md:border-4 border-primary bg-primary/10 flex items-center justify-center">
                                    <span className="material-symbols-outlined text-2xl md:text-4xl text-primary">military_tech</span>
                                </div>
                            </div>
                            <div className="absolute -bottom-1 left-1/2 -translate-x-1/2">
                                <span className="px-2 md:px-3 py-0.5 md:py-1 bg-primary text-white text-[10px] md:text-xs font-bold rounded-full shadow-lg">DIAMOND</span>
                            </div>
                        </div>

                        {/* Progress Info */}
                        <div className="flex-1 text-center md:text-left">
                            <h2 className="text-xl md:text-3xl font-black text-slate-900 mb-2">Next Rank: Blue Diamond</h2>
                            <p className="text-xs md:text-sm text-slate-500 mb-6 font-medium">
                                You're {progressPercent}% of the way to your next rank advancement. Keep it up!
                            </p>

                            {/* Progress Bar */}
                            <div className="mb-6 max-w-md mx-auto md:mx-0">
                                <div className="flex items-center justify-between mb-2">
                                    <span className="text-[10px] md:text-xs font-bold text-slate-600 uppercase tracking-wider">Total Progress</span>
                                    <span className="text-[10px] md:text-xs font-bold text-primary">{progressPercent}%</span>
                                </div>
                            ...
                            <div className="grid grid-cols-2 gap-4 max-w-md mx-auto md:mx-0">
                                <div className="bg-slate-50 rounded-xl p-3 md:p-4 border border-slate-100">
                                    <div className="text-[10px] md:text-xs font-bold text-slate-500 uppercase mb-1">Current PV</div>
                                    <div className="text-lg md:text-2xl font-black text-slate-900">
                                        {currentPV.toLocaleString()} <span className="text-[10px] md:text-xs text-slate-400 font-bold">/ {(targetPV / 1000)}k</span>
                                    </div>
                                </div>
                                <div className="bg-slate-50 rounded-xl p-3 md:p-4 border border-slate-100">
                                    <div className="text-[10px] md:text-xs font-bold text-slate-500 uppercase mb-1">Direct Legs</div>
                                    <div className="text-lg md:text-2xl font-black text-slate-900">
                                        {directLegs} <span className="text-[10px] md:text-xs text-slate-400 font-bold">/ {targetLegs}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div className="bg-slate-900 rounded-2xl p-6 md:p-8 text-white relative overflow-hidden shadow-xl">
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="text-3xl md:text-5xl font-black mb-6">$24,850<span className="text-base md:text-xl text-slate-500 ml-1">.00</span></div>

                        <div className="space-y-4 mb-8">
                            <div className="flex items-center gap-3">
                                <span className="material-symbols-outlined text-primary">emoji_events</span>
                                <span className="text-xs md:text-sm font-bold text-slate-300">12 Achievements Unlocked</span>
                            </div>
                            <div className="flex items-center gap-3">
                                <span className="material-symbols-outlined text-primary">card_giftcard</span>
                                <span className="text-xs md:text-sm font-bold text-slate-300">2 Rewards Pending Claim</span>
                            </div>
                        </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-6 md:p-8 shadow-sm">
                <h2 className="text-xl md:text-2xl font-black text-slate-900 mb-8 font-display">Career Roadmap</h2>
                    ...
                    <div className="relative flex flex-col md:flex-row justify-between gap-8 md:gap-0">
                        {careerRoadmap.map((item, index) => (
                            <div key={index} className="flex md:flex-col items-center gap-4 md:gap-3">
                                <div className={`w-12 h-12 rounded-full ${item.color} flex items-center justify-center ring-4 ring-white shadow-md relative z-10 ${item.current ? "ring-primary/20 scale-110" : ""
                                    }`}>
                                ...
                                <div className="text-left md:text-center">
                                    <div className={`font-black text-sm ${item.current ? "text-primary" : "text-slate-900 opacity-60"}`}>
                                        {item.rank}
                                    </div>
                                    <div className={`text-[10px] font-bold uppercase tracking-wider ${item.current ? "text-primary/70" : "text-slate-400"
                                        }`}>
                                        {item.status}
                                    </div>
                                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {rewards.map((reward) => (
                    <div
                        key={reward.id}
                        className={`${reward.bgColor} rounded-2xl border border-transparent hover:border-slate-200 transition-all p-6 relative group shadow-sm`}
                    >
                        {/* Status Badge */}
                        <div className="mb-6 flex justify-between items-start">
                            <span className={`${reward.statusColor} px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-wider`}>
                                {reward.status}
                            </span>
                            <span className={`material-symbols-outlined text-3xl ${reward.iconColor} opacity-20 group-hover:opacity-100 transition-opacity`}>
                                {reward.icon}
                            </span>
                        </div>
                        ...
                        <div className="text-2xl font-black text-slate-900 mb-3">{reward.value}</div>
                        <p className="text-xs text-slate-500 mb-6 font-medium leading-relaxed">{reward.description}</p>
                        ...
                        <div className="mt-auto pt-6 border-t border-slate-200/50">
                            {reward.showClaim ? (
                                ...
                                <div className="flex items-center gap-2 text-emerald-600">
                                    ...
                                    <span className="text-xs font-bold uppercase tracking-wider">{reward.claimedDate}</span>
                                </div>
                            ) : (
                                <div className="flex items-center gap-2 text-slate-400">
                                    <span className="material-symbols-outlined text-lg">lock</span>
                                    <span className="text-xs font-bold uppercase tracking-wider">{reward.lockText}</span>
                                </div>
                            )}
                        </div>
                    </div>
